# Program to find the outlying xg2 files.
...
# If passed is false, we need to find the worst file, and remove it  
   ...
   if not passed[0]: # the abs xsec has failed, remove the btl grid dat and top files
      ...
      top='pwg-xg2-'+seedToRemove+'-btlgrid.top'
      dat='pwggridinfo-btl-xg2-'+seedToRemove+'.dat'
      log='run-st1-xg2-'+str(int(seedToRemove))+'.log'
      if os.path.isfile(top):
         os.rename(top,top+'-bad-xg2-abs')
      if os.path.isfile(dat):
         os.rename(dat,dat+'-bad-xg2-abs')
      if os.path.isfile(log): # may have already been deleted in the previous section from abs
         os.rename(log,log+'-bad-xg2-abs')
   ...
   if not passed[1]: # remnant has failed, however, both could fail so we put them in two separate loops
      ...
      top='pwg-xg2-'+seedToRemove+'-rmngrid.top'
      dat='pwggridinfo-rmn-xg2-'+seedToRemove+'.dat'
      log='run-st1-xg2-'+str(int(seedToRemove))+'.log'
      if os.path.isfile(top):
         os.rename(top,top+'-bad-xg2-rem')
      if os.path.isfile(dat):
         os.rename(dat,dat+'-bad-xg2-rem')
      if os.path.isfile(log): # may have already been deleted in the previous section from abs
         os.rename(log,log+'-bad-xg2-rem')
