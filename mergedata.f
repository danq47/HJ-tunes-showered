...
c one per line, to be merged. An empty lines terminates the list.
      ...
      parameter (maxfiles=2000,maxlines=55000)
      ...
      do k=1,nlines(1)
         ...
         if(ios.ne.0) then
            ...
            do ifile=2,nfiles
               ...
               elseif(imethod.eq.6) then
               	y=y*((21.6*.216)/100.0) ! this is the branching ratio of ttbar to 2 leptons (mu and e) - scale down the NLO to just take these channels into account
               endif
            enddo
            if(imethod.eq.1) then
               y=y/nfiles
